...
void pal(string &s){
	...
	//	bool m=false;
		...
		if( (s.at(mid-1) == s.at(mid) &&  temp.compare(rev) == 0) || (temp.compare(rev) > 0 && s.at(mid-1)==s.at(mid) ) ){ //if already pal 
	//		m=true;
			if( s.at(mid-1) != '9' && ! (s.at(mid-1) > s.at(mid)) ){
				  s[mid-1] = s.at(mid-1)+1;
			}else if(s.at(mid-1) == '9') {
			next=mid-1;
			//s[next]='0';
			while( next >= 0 ){
				if ( s.at(next) == '9' )
