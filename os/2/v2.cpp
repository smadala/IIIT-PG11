...
void getPerm(string& perm,string& link,mode_t st_mode,char *path){
link="";
perm="";
if(S_ISLNK(st_mode))
{
...
getPerm(details.perm,details.link,buf.st_mode,fullPath);
...
for(vector<f_info>::iterator it=detailsV.begin();it!=detailsV.end();++it){
cout<< /*"\e[32m\e"<<*/setw(pl)<< it->perm << setw(hl) << it->hLink << setw(ul) << it->uName 
<< setw(gl) << it->gName << setw(sl) << it->size << setw(dl) << it->time <<" "<< it->name << it->link <<endl;
}
}
else
for(vector<f_info>::iterator it=detailsV.begin();it!=detailsV.end();++it){
